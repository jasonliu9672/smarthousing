const SmartHousing = artifacts.require("SmartHousing");
const bcrypt = require('bcrypt');
const saltRounds = 10;

contract("SmartHousing", accounts =>{
    let smartHousing;
    // let landlord_name = "Yuan Tai Liu";
    // let landlord_pid = "A123456789";
    var mainAccount = accounts[0]
    beforeEach(async () => {
        smartHousing = await SmartHousing.deployed();
    })
    it('should set account 1 as owner', async ()=>{
        let owner = await smartHousing.owner.call({from:mainAccount});
    ...
    //     let pay_day = "5";
    //     let contract_id = 0;
    //     return smartHousing.addNewContract(physical_address,city,zip_code,house_type,rent,term,pay_day,{from:mainAccount
    //     }).then(function(result) {
    //         return smartHousing.getContractHouseInfo(contract_id)
    ...
    //     let contract_id = 0;
    //     return smartHousing.requestLease(name,email,phone_number,contract_id,{from:account_three
    //     }).then(function(result){
    //         return smartHousing.selectPendingRequest(contract_id,account_three)
    //     }).then(function(result){
    //         return smartHousing.getContractStatus.call(contract_id);
    //     }).then(function(state){
